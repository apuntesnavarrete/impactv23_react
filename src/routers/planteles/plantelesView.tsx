...
const PlantelesTabla: React.FC = () => {
  ...
  // Estado para controlar el filtro de asistencias
  const [filterEnabled, setFilterEnabled] = useState(false);
  const [assistancesThreshold, setAssistancesThreshold] = useState<number>(5);
  ...
  return (
    <div>
      <div>
        ...
        />
        {/* Input para el número de asistencias */}
        <input
          type="number"
          placeholder="Número de asistencias"
          value={assistancesThreshold}
          onChange={(e) => setAssistancesThreshold(Number(e.target.value))}
        />
        {/* Botón para activar/desactivar el filtro */}
        <button onClick={() => setFilterEnabled(!filterEnabled)}>
          {filterEnabled ? 'Desactivar Filtro' : 'Activar Filtro'}
        </button>
      </div>
                ...
                if (goleador.id === item.participants.id) {
                  return (
                    <td key={`asistencia-${item.id}`}>
                      {filterEnabled ? (goleador.asistencias >= assistancesThreshold ? "Autorizado a liguilla" : "") : goleador.asistencias}
                    </td>
                  );
                }
                return null;
            ...
            <div className="card_interna">
              ...
              <div className="card_data">
                ...
                <a href={`Edit/${player.id}`}>
                  ...
                  {player.participants.birthDate && (
  <p className="card_Dorsal">
    Año .- {new Date(player.participants.birthDate).getFullYear()}
  </p>
)}
                </a>
                {goleadoresFiltrados.map((goleador) => {
                  if (goleador.id === player.participants.id) {
                    return (
                      <p key={`asistencia-${player.id}`} className="card_asistencia">
                        Asistencias: {filterEnabled ? (goleador.asistencias >= assistancesThreshold ? "Autorizado a liguilla" : "") : goleador.asistencias}
                      </p>
                    );
                  }
                  return null;
